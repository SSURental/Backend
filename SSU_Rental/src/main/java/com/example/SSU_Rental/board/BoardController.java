...
public class BoardController {
    ...
    @Operation(summary = "게시글 작성 요청",parameters={@Parameter(in = ParameterIn.HEADER,name = "X-AUTH-TOKEN",required = true,description = "발급받은 JWT토큰")})
    @ApiResponses(value = {
    ...
    @Operation(summary = "게시글 좋아요 요청",parameters={@Parameter(in = ParameterIn.HEADER,name = "X-AUTH-TOKEN",required = true,description = "발급받은 JWT토큰")})
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200",description = "게시글 좋아요", content = @Content(schema = @Schema(implementation = Long.class)))
    })
    @PostMapping("/boards/{boardId}/like")
    public ResponseEntity<Long> like(@Parameter(description = "게시글 ID",required = true) @PathVariable Long boardId) {
        boardService.like(boardId);
        return ResponseEntity.ok().body(boardId);
    }

    @Operation(summary = "게시글 싫어요 요청",parameters={@Parameter(in = ParameterIn.HEADER,name = "X-AUTH-TOKEN",required = true,description = "발급받은 JWT토큰")})
    @ApiResponses(value = {
    ...
    public ResponseEntity<Long> dislike(@Parameter(description = "게시글 ID",required = true) @PathVariable Long boardId) {
        boardService.dislike(boardId);
        return ResponseEntity.ok().body(boardId);
    }

    @Operation(summary = "게시글 경고 요청",parameters={@Parameter(in = ParameterIn.HEADER,name = "X-AUTH-TOKEN",required = true,description = "발급받은 JWT토큰")})
    @ApiResponses(value = {
    ...
    public ResponseEntity<Long> warn(@Parameter(description = "게시글 ID",required = true)  @PathVariable Long boardId) {
    ...
    public ResponseEntity<BoardResponse> getOne(@Parameter(description = "게시글 ID",required = true) @PathVariable Long boardId) {

        return ResponseEntity.ok().body(boardService.getOne(boardId));
    }

    /**
     * /boards ? page =1& size=5& sort=id -> 최신 순대로 정렬
     * /boards ? page =1& size=5& sort=see_cnt -> 조회수 대로 정렬
...
//    }
    ...
    public ResponseEntity delete(@Parameter(description = "게시글 ID",required = true) @PathVariable Long boardId, @Parameter(hidden = true)   @AuthMember Member member) {
